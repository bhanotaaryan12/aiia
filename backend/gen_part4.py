"""
AIIA CTMS Backend Generator - Part 4: Seed Data
"""
import os
...
async def seed_data():
    ...
    async with AsyncSessionLocal() as db:
        ...
        dqs = [
            DataQuery(submission_id=None, query_text="Blood pressure reading seems high - please verify", query_type="MANUAL", status="OPEN", raised_by=users["DATA_MANAGER"].id),
            DataQuery(submission_id=None, query_text="Missing weight measurement", query_type="AUTO_VALIDATION", status="OPEN", raised_by=users["DATA_MANAGER"].id),
            DataQuery(submission_id=None, query_text="VAS score exceeds expected range", query_type="MANUAL", status="ANSWERED", raised_by=users["DATA_MANAGER"].id, answer_text="Score verified by PI"),
        ]
        # We can just store them without submission_id for now
        # In production they would link to real submissions
        db.add_all(dqs)
        await db.flush()
        ...
        notif_data = [
            ...
            (users["PHARMACOVIGILANCE_OFFICER"], "Safety Signal Detected", "Potential safety signal: Elevated liver enzymes cluster", "SAE_REPORTED", "URGENT"),
...
print("Part 4 (Seed Data) generated successfully.")
